...
function renderizarFrete() {
  const usuarioGestorDono = dadosUsuario.tipo === "gestor" && freteAtual.gestorId === usuarioAtual.uid;
  const motoristaAprovado = freteAtual.motoristaAprovadoId === usuarioAtual.uid;
  const podeVerEndereco = usuarioGestorDono || motoristaAprovado;
  ...
  if (podeVerEndereco) {
    ...
    html += `
      <hr>
      <p><strong>Endereço completo:</strong> liberado somente após aprovação do gestor.</p>
    `;
  }

  if (usuarioGestorDono) {
    html += `
      <hr>
      <div class="actions">
        <button class="btn primary" onclick="alterarStatusFrete('em_coleta')">Marcar em coleta</button>
        <button class="btn primary" onclick="alterarStatusFrete('em_viagem')">Marcar em viagem</button>
        <button class="btn primary" onclick="alterarStatusFrete('concluido')">Marcar concluído</button>
...
async function carregarInteressados() {
  ...
  if (snap.empty) {
    listaInteressados.innerHTML = "<p>Nenhum caminhoneiro interessado ainda.</p>";
    return;
  }

  listaInteressados.innerHTML = "";

  for (const interesseDoc of snap.docs) {
    const interesse = interesseDoc.data();
    await renderizarInteressado(interesseDoc.id, interesse);
  }
}

async function renderizarInteressado(interesseId, interesse) {
  const usuarioRef = doc(db, "usuarios", interesse.motoristaId);
  const usuarioSnap = await getDoc(usuarioRef);

  const motoristaRef = doc(db, "motoristas", interesse.motoristaId);
  ...
  card.innerHTML = `
    ...
    <p><strong>Cidade atual:</strong> ${motorista.cidadeAtual || "-"} / ${motorista.estadoAtual || "-"}</p>
    <p><strong>Disponível:</strong> ${motorista.disponivel ? "Sim" : "Não"}</p>
    <p><strong>Caminhão:</strong> ${motorista.tipoCaminhao || "-"}</p>
    <p><strong>Carroceria:</strong> ${motorista.carroceria || "-"}</p>
    <p><strong>Capacidade:</strong> ${motorista.capacidade || "-"}</p>
    <p><strong>Placa:</strong> ${motorista.placa || "-"}</p>

    <div class="actions">
      <button class="btn primary">Aprovar motorista</button>
      ${
        whatsappLimpo
          ? `<a class="btn secondary" target="_blank" href="https://wa.me/55${whatsappLimpo}?text=Olá, vi seu interesse no frete ${encodeURIComponent(freteAtual.origemCidade + " para " + freteAtual.destinoCidade)}">Chamar no WhatsApp</a>`
          : ""
      }
    </div>
  `;

  const botaoAprovar = card.querySelector("button");
  botaoAprovar.addEventListener("click", () => aprovarMotorista(interesseId, interesse.motoristaId));

  listaInteressados.appendChild(card);
}

async function aprovarMotorista(interesseId, motoristaId) {
  const confirmar = confirm("Deseja aprovar este motorista para o frete?");
  if (!confirmar) return;
  ...
  try {
    ...
    alert("Motorista aprovado! Endereço e rotas foram liberados para ele.");
    window.location.reload();
  } catch (erro) {
    alert("Erro ao aprovar motorista: " + erro.message);
  }
}

window.alterarStatusFrete = async function (novoStatus) {
  try {
    await updateDoc(doc(db, "fretes", freteId), {
      status: novoStatus
    });
